...
def init_database():
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute("CREATE TABLE IF NOT EXISTS schema_migrations (version INTEGER PRIMARY KEY, applied_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::BIGINT)")
            cursor.execute("SELECT version FROM schema_migrations ORDER BY version")
            applied = {row["version"] for row in cursor.fetchall()}

            if 1 not in applied:
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS accounts (id BIGSERIAL PRIMARY KEY, name TEXT, username TEXT, session TEXT UNIQUE, enabled BOOLEAN NOT NULL DEFAULT TRUE);
            ...
            if 2 not in applied:
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS users (id BIGSERIAL PRIMARY KEY, username TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'user' CHECK (role IN ('user', 'admin')), enabled BOOLEAN NOT NULL DEFAULT TRUE, created_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::BIGINT);
            ...
            if 3 not in applied:
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS resources (id BIGSERIAL PRIMARY KEY, identity_key TEXT UNIQUE NOT NULL, filename TEXT NOT NULL, size BIGINT NOT NULL DEFAULT 0, mime_type TEXT, status TEXT NOT NULL DEFAULT 'active', created_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::BIGINT, updated_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::BIGINT);
                    ALTER TABLE files ADD COLUMN IF NOT EXISTS resource_id BIGINT;
                    INSERT INTO resources(identity_key, filename, size, mime_type)
                    SELECT md5(lower(trim(filename)) || '|' || size::TEXT || '|' || COALESCE(mime_type, '')), min(filename), max(size), max(mime_type) FROM files
                    GROUP BY md5(lower(trim(filename)) || '|' || size::TEXT || '|' || COALESCE(mime_type, '')) ON CONFLICT (identity_key) DO NOTHING;
                    UPDATE files f SET resource_id = r.id FROM resources r WHERE r.identity_key = md5(lower(trim(f.filename)) || '|' || f.size::TEXT || '|' || COALESCE(f.mime_type, '')) AND f.resource_id IS NULL;
            ...
            if 4 not in applied:
                cursor.execute("""
                    ...
                    CREATE TABLE IF NOT EXISTS resource_categories (resource_id BIGINT NOT NULL REFERENCES resources(id) ON DELETE CASCADE, category_id BIGINT NOT NULL REFERENCES categories(id) ON DELETE CASCADE, created_at BIGINT NOT NULL DEFAULT EXTRACT(EPOCH FROM NOW())::BIGINT, PRIMARY KEY (resource_id, category_id));
                    CREATE INDEX IF NOT EXISTS idx_resource_categories_category ON resource_categories(category_id);
                    INSERT INTO resource_categories(resource_id, category_id) SELECT DISTINCT f.resource_id, f.category_id FROM files f WHERE f.resource_id IS NOT NULL AND f.category_id IS NOT NULL ON CONFLICT DO NOTHING;
                """)
                cursor.execute("INSERT INTO schema_migrations(version) VALUES (4)")

            if 5 not in applied:
                cursor.execute("ALTER TABLE files DROP COLUMN IF EXISTS category_id")
            ...
            if 7 not in applied:
                cursor.execute("""
                    UPDATE resources
                    SET identity_key = 'index:' || lower(trim(filename)) || '|' || size::TEXT || '|' || COALESCE(mime_type, '')
