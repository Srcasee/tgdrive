        ...
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    ...
                    SET status = 'completed', completed_at = %s, bytes_transferred = %s
                    WHERE id = %s AND status = 'active'
                    """,
                    (now, bytes_transferred, record_id),
                )
            conn.commit()
        ...
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    UPDATE download_records
                    SET status = 'failed', completed_at = %s, bytes_transferred = %s, error = %s
                    WHERE id = %s AND status = 'active'
                    """,
                    (now, bytes_transferred, error[:2000], record_id),
                )
            conn.commit()

    def list_active(self, limit=100):
        return self._list("WHERE status = 'active'", limit)

    def list_history(self, limit=100):
        return self._list("WHERE status IN ('completed', 'failed')", limit)
