...
async function renderSources(container) {
  ...
  try {
    ...
    const rows=[];
    for(const source of sources){ rows.push([source.id,source.name||source.telegram_chat_id,source.account_name,source.telegram_chat_id,source.enabled?'运行中':'已禁用',source.scan_status||'idle']); }
    const wrap=panel('来源列表'); wrap.appendChild(table(['ID','名称','Telegram 账号','Chat ID','状态','扫描状态'],rows));
    const actions=document.createElement('div'); actions.className='toolbar';
    for(const source of sources){ const button=document.createElement('button'); button.type='button'; button.textContent=`${source.enabled?'禁用':'启用'} #${source.id}`; button.onclick=async()=>{button.disabled=true;try{await setSourceEnabled(source.id,!source.enabled);const index=sources.indexOf(source);source.enabled=!source.enabled;const row=wrap.querySelector('tbody').children[index];row.children[4].textContent=source.enabled?'运行中':'已禁用';button.textContent=`${source.enabled?'禁用':'启用'} #${source.id}`;}catch(e){alert(e.message);}finally{button.disabled=false;}}; actions.appendChild(button); }
    wrap.appendChild(actions); list.appendChild(wrap);
...
async function renderFiles(container) {
  ...
  async function refresh(){ list.textContent='正在加载……'; try{const data=await listFiles(page,size,category.value); const files=data.items||[]; list.replaceChildren();
      ...
      const rows=files.map(file=>[file.id,file.filename||`Resource #${file.id}`,formatSize(file.size),file.mime_type||'未知',file.source_count??0,(file.category_ids||[]).map(id=>categories.find(c=>Number(c.id)===Number(id))?.name).filter(Boolean).join('、')||'未分类']);
      if(files.length) wrap.appendChild(table(['ID','文件名','大小','类型','Telegram 来源','分类'],rows)); else wrap.appendChild(document.createTextNode('暂无资源'));
      if(files.length){ const actions=document.createElement('div'); actions.className='actions'; files.forEach((file,index)=>{ const download=document.createElement('a'); download.href=`/resources/${file.id}/download`; download.textContent='下载'; download.setAttribute('download',''); actions.appendChild(download); const edit=document.createElement('button'); edit.type='button'; edit.textContent='分类'; edit.onclick=async()=>{const selected=prompt('输入分类 ID，多个分类用逗号分隔：',(file.category_ids||[]).join(','));if(selected===null)return;const ids=selected.split(',').map(v=>Number(v.trim())).filter(Number.isInteger);try{await setResourceCategories(file.id,ids);await refresh();}catch(error){alert(error.message);}};actions.appendChild(edit);}); wrap.appendChild(actions); }
...
async function renderCategories(container) {
  ...
  async function refresh(){const categories=await listCategories();list.replaceChildren();if(!categories.length){list.textContent='暂无分类';return;}const wrap=panel(`分类列表（${categories.length} 项）`);const rows=categories.map(c=>[c.id,c.name]);wrap.appendChild(table(['ID','分类名称'],rows));const actions=document.createElement('div');actions.className='actions';for(const category of categories){const rename=document.createElement('button');rename.type='button';rename.textContent=`修改 #${category.id}`;rename.onclick=async()=>{const value=prompt('分类名称',category.name);if(!value||value.trim()===category.name)return;try{await updateCategory(category.id,value.trim());await refresh();}catch(error){alert(error.message);}};const remove=document.createElement('button');remove.type='button';remove.className='danger';remove.textContent=`删除 #${category.id}`;remove.onclick=async()=>{if(!confirm(`确定删除分类“${category.name}”吗？`))return;try{await removeCategory(category.id);await refresh();}catch(error){alert(error.message);}};actions.append(rename,remove);}wrap.appendChild(actions);list.appendChild(wrap);}
