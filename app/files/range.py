...
def parse_single_range(value: str | None, size: int) -> tuple[int, int, bool]:
    ...
    Multiple ranges are deliberately rejected because the current streaming
    endpoints return a single byte stream and do not implement multipart/byteranges.
    """
    if size < 0:
    ...
    try:
        if start_text:
            ...
            if start < 0:
                raise InvalidRange("negative start")
            if start >= size:
